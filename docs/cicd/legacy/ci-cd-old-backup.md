...
- **접근**: Cloudflare Tunnel → `simulation.LOLCLUB.com`
...
- **Production**: 포트 3000 (Frontend), 8000 (Backend) → `simulation.LOLCLUB.com`
- **Staging**: 포트 4173 (Frontend), 8001 (Backend) → `staging-simulation.LOLCLUB.com`
- **배포**: 자동화 (GitHub Actions)
- **고가용성**: 무중단 배포, Health Check

**⚠️ 중요 변경사항**: Staging 도메인을 `staging.simulation` → `staging-simulation`으로 변경했습니다. 이는 Cloudflare Universal SSL이 다단계 서브도메인(multi-level subdomain)을 지원하지 않아 `ERR_SSL_VERSION_OR_CIPHER_MISMATCH` 오류가 발생했기 때문입니다. 단일 레벨 서브도메인(`staging-simulation.LOLCLUB.com`)은 무료 Universal SSL 인증서로 자동 커버됩니다.
...
│  │              Nginx (Port 8080)                        │   │
│  │  Host-based Routing:                                  │   │
│  │  - simulation.LOLCLUB.com → localhost:3000   │   │
│  │  - staging-simulation.LOLCLUB.com → :4173    │   │
│  └──────┬────────────────────────────┬──────────────────┘   │
│         │                            │                       │
│         ▼                            ▼                       │
│  ┌─────────────────┐         ┌─────────────────┐            │
...
- 도메인: `LOLCLUB.com` (이미 Cloudflare에 등록되어 있음)
- 서브도메인: `simulation.LOLCLUB.com` (현재 Windows로 라우팅 중)
- 서브도메인: `staging-simulation.LOLCLUB.com` (신규 생성 필요)
...
- [ ] 현재 서비스 접속 가능 확인 (`https://simulation.LOLCLUB.com`)
...
1. **즉시 롤백** (5-10분 소요):
   - Cloudflare Dashboard → DNS 설정 → `simulation.LOLCLUB.com`의 Tunnel을 Windows Tunnel로 재전환
...
| 단계          | 도메인                           | 시점                | 목적                                   |
| ------------- | -------------------------------- | ------------------- | -------------------------------------- |
| **섹션 6.5**  | `staging-simulation.LOLCLUB.com` | **지금**            | Staging 환경 테스트용                  |
| **섹션 12.4** | `simulation.LOLCLUB.com`         | **마이그레이션 시** | Production 전환 (Windows 터널 중단 후) |
...
**⚠️ 중요**: 여기서는 **Staging 도메인만** 생성합니다. Production 도메인(`simulation.LOLCLUB.com`)은 현재 Windows 터널이 사용 중이므로, 섹션 12.4에서 마이그레이션할 때 전환합니다.

**⚠️ SSL 인증서 이슈 해결**: 원래 `staging.simulation.LOLCLUB.com`을 사용하려 했으나, Cloudflare Universal SSL은 다단계 서브도메인(multi-level subdomain)을 지원하지 않습니다. 따라서 `staging-simulation.LOLCLUB.com`으로 변경합니다.
...
- staging-simulation.LOLCLUB.com (CNAME to <TUNNEL_ID>.cfargotunnel.com) ← 새로 생성됨
- simulation.LOLCLUB.com (기존 Windows 터널 유지) ← 아직 건드리지 않음
```

**왜 도메인을 변경했나요?**

- ❌ `staging.simulation.LOLCLUB.com` (3단계 서브도메인) → Universal SSL 미지원 → `ERR_SSL_VERSION_OR_CIPHER_MISMATCH` 오류
- ✅ `staging-simulation.LOLCLUB.com` (2단계 서브도메인) → Universal SSL 자동 지원

**Production DNS는 아직 생성/변경하지 마세요!** Windows 터널이 `simulation.LOLCLUB.com`을 계속 서비스하고 있습니다.
...
1. **New repository variable** 클릭 후 다음 추가:
   - `VITE_API_BASE_URL`: `https://staging-simulation.LOLCLUB.com/api`
...
# Staging 환경에서 실행
env:
  BASE_URL: https://staging-simulation.LOLCLUB.com
...
- <https://staging-simulation.LOLCLUB.com> (새 Droplet 터널)
...
# 브라우저에서 접속
Start-Process "https://simulation.LOLCLUB.com"
...
**중요**: Windows 터널을 중단한 이 시점부터 `simulation.LOLCLUB.com`은 일시적으로 접속 불가 상태가 됩니다 (5-10분). 다음 단계에서 즉시 Droplet 터널로 DNS를 전환합니다.
...
cloudflared tunnel route dns simulation-tunnel simulation.LOLCLUB.com
```
...
```text
Failed to add route: code: 1003, reason: Failed to create record simulation.LOLCLUB.com
...
2. 도메인 `LOLCLUB.com` 선택
3. **DNS → Records** 이동
4. `simulation.LOLCLUB.com` CNAME 레코드 찾기
5. **Edit** 클릭
...
- 기존 (Windows): `simulation.LOLCLUB.com` → `abc12345-old.cfargotunnel.com`
- 변경 후 (Droplet): `simulation.LOLCLUB.com` → `3a97bb01-new.cfargotunnel.com`
...
3. **DNS → Records** 이동
4. 다음 레코드 확인:
   - `simulation.LOLCLUB.com` → CNAME `<DROPLET_TUNNEL_ID>.cfargotunnel.com` ✅
   - `staging-simulation.LOLCLUB.com` → CNAME `<DROPLET_TUNNEL_ID>.cfargotunnel.com` ✅
...
# Staging도 확인
https://staging-simulation.LOLCLUB.com
```
...
- [x] `https://simulation.LOLCLUB.com` 접속 성공
- [x] 로그인 및 모든 기능 정상 작동
...
- ❌ `staging.simulation.LOLCLUB.com` (3단계 서브도메인) → SSL 인증서 미지원 (변경 전)
- ✅ `staging-simulation.LOLCLUB.com` (2단계 서브도메인) → Universal SSL 자동 지원 (변경 후)
...
2. **Cloudflare Dashboard에서 이전 레코드 삭제**:
   - Cloudflare Dashboard → DNS → Records
   - 이전 `staging.simulation.LOLCLUB.com` CNAME 레코드 삭제 (다단계 서브도메인)
   - 새 `staging-simulation.LOLCLUB.com` CNAME 레코드 확인 (orange cloud 활성화)
...
5. **GitHub Actions 환경 변수 업데이트**:
   - GitHub Repository → Settings → Secrets and variables → Actions → Variables
   - `VITE_API_BASE_URL` (staging): `https://staging-simulation.LOLCLUB.com/api`로 변경
...
6. **검증**:
   ...
   # curl로 SSL 인증서 확인
   curl -vI https://staging-simulation.LOLCLUB.com 2>&1 | grep -i "SSL\|certificate"
   ```
...
```
staging-simulation.LOLCLUB.com/api/otp/send:1 Failed to load resource: net::ERR_NAME_NOT_RESOLVED
...
5. **검증**:
   ...
   - API 호출이 올바른 도메인(`staging-simulation.LOLCLUB.com`)으로 가는지 확인
...
# 출력: VITE_API_BASE_URL=https://staging-simulation.LOLCLUB.com/api
...
# hostname이 staging-simulation.LOLCLUB.com인지 확인
...
- 설정: 5분마다 `https://simulation.LOLCLUB.com/health` 체크
...
| `VITE_API_BASE_URL`             | Variable | 프론트엔드 API URL        | `https://simulation.LOLCLUB.com/api` |
...
Production과 동일하되, `VITE_API_BASE_URL`은 `https://staging-simulation.LOLCLUB.com/api`
...
1. 사용자 → `simulation.LOLCLUB.com` (HTTPS:443)
2. Cloudflare Tunnel → Droplet Nginx (Port 8080)
3. Nginx → Host header 확인
   - `simulation.LOLCLUB.com` → `localhost:3000` (Production)
   - `staging-simulation.LOLCLUB.com` → `localhost:4173` (Staging)
...
- [ ] 도메인 접속 확인 (https://simulation.LOLCLUB.com)
...
- Health check: `curl https://simulation.LOLCLUB.com/health`
