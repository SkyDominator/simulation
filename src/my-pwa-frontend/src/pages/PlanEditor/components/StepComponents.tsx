...
function SalesRateInput({ value, onChange, onTabNext, inputRef }: { value: number; onChange: (val: number) => void; onTabNext?: () => boolean; inputRef?: (el: HTMLInputElement | null) => void; }) {
  const MIN = 50;
  const MAX = 100;
  ...
  const step = (dir: 1 | -1) => {
    const current = rawDigits === '' ? internalValue : (parseInt(rawDigits, 10) || internalValue);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    else if (e.key === 'Enter') { commitValue(rawDigits); }
  };
    ...
    <div className="flex items-center gap-1">
      ...
      <div className="flex gap-1">
        <button
          ...
          onClick={() => step(-1)}
          disabled={(rawDigits === '' ? internalValue : parseInt(rawDigits, 10)) <= MIN}
        >−</button>
        <button
          type="button"
          aria-label="증가"
          className="w-10 h-10 flex items-center justify-center rounded-md border border-gray-300 bg-gray-100 hover:bg-gray-200 active:bg-gray-300 text-lg font-medium"
          onClick={() => step(1)}
          disabled={(rawDigits === '' ? internalValue : parseInt(rawDigits, 10)) >= MAX}
        >+</button>
      </div>
    </div>
  );
...
const InvestmentTable: React.FC<InvestmentEditorProps> = ({ investments, companyRound, planType, onInvestmentChange, salesAchievementRates, onSalesRateChange }) => {
                ...
                <tr key={inv.round} className="bg-white border-b">
                  ...
                  <td className="px-6 py-4">
                    <FormattedAmountInput
                      ...
                      inputRef={(el: HTMLInputElement | null) => { if (el) amountRefs.current[index] = el; }}
                    />
                  </td>
                  <td className="px-10 py-4">
                    <SalesRateInput
                      ...
                      inputRef={(el: HTMLInputElement | null) => { if (el) rateRefs.current[index] = el; }}
                    />
                  </td>
                </tr>
              );
