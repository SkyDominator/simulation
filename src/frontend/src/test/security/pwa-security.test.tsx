...
describe("PWA Security Tests", () => {
  ...
  describe("Service Worker Security", () => {
    ...
    it("should validate service worker script integrity", () => {
      ...
      const validPaths = ["/sw.js", "/service-worker.js", "/static/sw.js"];
  ...
  describe("Offline Security", () => {
    ...
    it("should validate offline request integrity", () => {
      const validateOfflineRequest = (request: Request) => {
        ...
        const blockedPaths = ["/api/admin/", "/api/auth/", "/api/payment/"];
        ...
        if (!["GET", "HEAD"].includes(request.method)) {
          ...
          if (
            !allowedWritePaths.some((path) => url.pathname.startsWith(path))
          ) {
      ...
      const validRequests = [
        ...
        new Request(new URL("/api/sync", window.location.origin), {
          method: "POST",
        }),
      ...
      const invalidRequests = [
        ...
        new Request(new URL("/api/simulations", window.location.origin), {
          method: "DELETE",
        }), // Write to non-sync endpoint
      ];
