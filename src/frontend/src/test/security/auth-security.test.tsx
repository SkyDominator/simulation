...
const MockAuthProvider = ({ children, mockUser = null, mockSession = null }: any) => {
...
const ProtectedComponent = () => {
  ...
  return <div data-testid="protected-content">Protected Content: {user.id}</div>
}
...
describe('Authentication Security Tests', () => {
  ...
  describe('Access Control', () => {
    ...
    it('should allow access to protected content with valid authentication', () => {
      ...
      const mockSession = { access_token: 'valid-token', expires_at: Date.now() + 3600000 }
    ...
    it('should handle authentication state changes properly', async () => {
      ...
      const mockSession = { access_token: 'new-token', expires_at: Date.now() + 3600000 }
  ...
  describe('Token Security', () => {
    ...
    it('should handle expired tokens properly', () => {
      ...
      expect(screen.getByTestId('token-status')).toHaveTextContent('Token Expired')
    })
  ...
  describe('Session Management', () => {
      ...
      const DataStorageComponent = () => {
        const handleStoreData = () => {
          ...
          Object.entries(sensitiveData).forEach(([key, value]) => {
            const sensitiveKeys = ['password', 'creditCard', 'ssn', 'token', 'secret']
    ...
    it('should handle concurrent session validation', async () => {
        ...
        return (
          <div>
            {validationResults.map((result, i) => (
              <div key={i} data-testid={`validation-${i}`}>{result}</div>
            ))}
          </div>
        )
      }
      ...
      await waitFor(() => {
        for (let i = 0; i < 5; i++) {
          expect(screen.getByTestId(`validation-${i}`)).toHaveTextContent('Valid')
        }
      }, { timeout: 1000 })
    })
  })
  ...
  describe('Authorization Security', () => {
    it('should enforce role-based access control', () => {
      ...
      const RoleBasedComponent = ({ requiredRole }: { requiredRole: string }) => {
        ...
        if (!user) return <div data-testid="login-required">Login required</div>
        if (!hasRole) return <div data-testid="access-denied">Access denied</div>
        
        return <div data-testid="access-granted">Access granted for {requiredRole}</div>
      }
    ...
    it('should validate user permissions for specific actions', () => {
      ...
      const PermissionComponent = ({ requiredPermission }: { requiredPermission: string }) => {
        const { user } = useAuth()
