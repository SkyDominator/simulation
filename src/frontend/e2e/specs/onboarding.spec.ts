...
// Legacy tests use old helper architecture for backward compatibility
base.describe("User Onboarding Flow", () => {
  ...
  base("shows an error for non-whitelisted users", async ({ page }) => {
  ...
  base(
    "surfaces invalid OTP errors without advancing the flow",
    async ({ page }) => {
  ...
  base(
    "E2E-PREAUTH-005: Resend button triggers new OTP send with rate limiting",
    async ({ page }) => {
      await page.goto("/");
  ...
  base(
    "E2E-PREAUTH-007: Consent page shows privacy policy content with network call",
    async ({ page }) => {
      await page.goto("/");
  ...
  base(
    "E2E-PREAUTH-009: Accept consent proceeds to login page with API call",
    async ({ page }) => {
  ...
  base(
    "E2E-PREAUTH-011: Back button from login returns to whitelist page",
    async ({ page }) => {
      await page.goto("/");
