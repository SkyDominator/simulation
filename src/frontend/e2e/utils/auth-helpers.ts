...
export async function loginTestUser(page: Page) {
  ...
  await page.addInitScript(() => {
    ...
    // Mock session state
    window.sessionStorage.setItem('ui.page', '"main"')
  })
}
...
export async function completeOnboardingFlow(
  page: Page, 
  userData: { name: string; phone: string } = { name: '홍길동', phone: '010-1234-5678' }
) {
  ...
  const consentExists = await page.locator('[data-testid="consent-checkbox"]').isVisible().catch(() => false)
  if (consentExists) {
