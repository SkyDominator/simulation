---
applyTo: '**/*.py'
---

# Instructions for Python AI Code Assistant

## General Guidelines

1. **Code Quality**: Ensure the code is clean, well-structured, and follows Python best practices. 
   - Use meaningful variable and function names.
   - Keep functions short and focused on a single task.
   - Avoid deep nesting of code blocks.
   - Use logging instead of print statements for debugging and information output.
   - Use type hints to improve code readability and help with static analysis.
2. **Documentation**: Add docstrings to functions and classes, and inline comments where necessary to explain complex logic.
3. **Error Handling**: Implement appropriate error handling to manage exceptions gracefully.
4. **Performance**: Optimize code for performance where applicable, but prioritize readability and maintainability.
5. **Dependencies**: If new libraries are introduced, ensure they are documented and included in the requirements file.
6. **Version Control**: Use meaningful commit messages that describe the changes made.
7. **Code Style**: Follow PEP 8 guidelines for Python code style, including naming conventions, indentation, and line length.
8. **Security**: Be mindful of security best practices, especially when handling user input or sensitive data.
9. **Refactoring**: If you encounter code that can be improved, refactor it while ensuring existing functionality remains intact.
10. **Functionality**: Ensure that the code meets the specified requirements and behaves as expected.
11. **Dependencies**: If new libraries are introduced, ensure they are documented and included in the requirements file.
